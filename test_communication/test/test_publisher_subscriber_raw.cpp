...
TEST_F(CLASSNAME(TestMessageSerialization, RMW_IMPLEMENTATION), raw_callback) {
  ...
  auto raw_callback = [&counter](const std::shared_ptr<rmw_message_raw_t> raw_msg) {
      ...
      for (auto i = 0u; i < raw_msg->buffer_length; ++i) {
      ...
      auto ret =
        rmw_deserialize(raw_msg.get(), message_cpp_typesupport, primitive_msg.get());
      ASSERT_EQ(RMW_RET_OK, ret);
      EXPECT_EQ(counter, primitive_msg->uint8_value);
      counter++;
    };

  auto node = rclcpp::Node::make_shared("test_publisher_subscriber_raw");
  auto subscriber = node->create_subscription<test_msgs::msg::Primitives>(
    "test_publisher_subscriber_raw_topic", raw_callback);
  auto publisher = node->create_publisher<test_msgs::msg::Primitives>(
    "test_publisher_subscriber_raw_topic");
